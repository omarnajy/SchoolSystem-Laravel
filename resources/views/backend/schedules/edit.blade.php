{{-- resources/views/backend/schedules/edit.blade.php --}}
@extends('layouts.app')
...
@section('content')
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="flex items-center justify-between mb-8">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 mb-2">Modifier l'Emploi du Temps</h1>
                <p class="text-gray-600">Modifiez les informations du cours</p>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <form method="POST" action="{{ route('schedules.update', $schedule->id) }}" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="start_time" id="start_time" required
                            ...
                            @foreach ($timeSlots as $time)
                                <option value="{{ $time }}"
                                    {{ old('start_time', $schedule->formatted_start_time) == $time ? 'selected' : '' }}>
                                    {{ $time }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="end_time" id="end_time" required
                            ...
                            @foreach ($timeSlots as $time)
                                <option value="{{ $time }}"
                                    {{ old('end_time', $schedule->formatted_end_time) == $time ? 'selected' : '' }}>
                                    {{ $time }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="academic_year" id="academic_year" required
                            ...
                            <option value="{{ date('Y') }}"
                                {{ old('academic_year', $schedule->academic_year) == date('Y') ? 'selected' : '' }}>
                                {{ date('Y') }}</option>
                            <option value="{{ date('Y') + 1 }}"
                                {{ old('academic_year', $schedule->academic_year) == date('Y') + 1 ? 'selected' : '' }}>
                                {{ date('Y') + 1 }}</option>
                        </select>
                        @error('academic_year')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <select name="semester" id="semester" required
                            ...
                            <option value="1" {{ old('semester', $schedule->semester) == '1' ? 'selected' : '' }}>
                                Semestre 1</option>
                            <option value="2" {{ old('semester', $schedule->semester) == '2' ? 'selected' : '' }}>
                                Semestre 2</option>
                        </select>
                        @error('semester')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="mt-6">
                    ...
                    <textarea name="notes" id="notes" rows="3" placeholder="Notes additionnelles sur ce cours..."
                        class="w-full border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('notes') border-red-500 @enderror">{{ old('notes', $schedule->notes) }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
